...
<div class="card p-3 p-md-4" style="max-width:620px">
    ...
    <form action="{{ route('participants.update', $participant) }}" method="POST" id="formEditParticipant" novalidate>
        @csrf @method('PUT')
        {{-- Nama --}}
        <div class="mb-3">
...
(function () {
    ...
    // Real-time events + pre-validate existing values
    Object.keys(rules).forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        el.addEventListener('input', () => validateField(id));
        el.addEventListener('blur',  () => validateField(id));
        if (el.value) validateField(id); // pre-validate populated values
    });

    // On submit
    document.getElementById('formEditParticipant').addEventListener('submit', function (e) {
        let valid = true;
